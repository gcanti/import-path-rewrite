...
function modifyFile(f) {
    return function (path) {
        return pipeable_1.pipe(readFile(path, 'utf8'), TE.map(f), TE.chain(function (content) { return writeFile(path, content); }), TE.chain(function () { return TE.rightIO(Console_1.log(path + " rewritten")); }));
    };
}
...
var replaceFiles = pipeable_1.pipe(modify(ES6_GLOB_PATTERN), TE.chain(function () { return modify(DIST_ES6_GLOB_PATTERN); }));
